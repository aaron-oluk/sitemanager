            ...
            <form method="POST" action="{{ route('emails.update', $email) }}" class="bg-white rounded-2xl shadow border border-gray-100 p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="email" name="email_address" value="{{ old('email_address', $email->email_address) }}" required class="mt-1 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input name="provider" value="{{ old('provider', $email->provider) }}" required class="mt-1 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        ...
                        <select name="hosting_plan" class="mt-1 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-blue-500">
                            @foreach(['monthly' => 'Monthly', 'quarterly' => 'Quarterly', 'biannual' => 'Bi-Annual', 'annual' => 'Annual', 'biennial' => 'Bi-Annual (2 Years)', 'triennial' => 'Tri-Annual (3 Years)'] as $value => $label)
                                <option value="{{ $value }}" {{ old('hosting_plan', $email->hosting_plan) == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Monthly Cost</label>
                        <input type="number" step="0.01" name="monthly_cost" value="{{ old('monthly_cost', $email->monthly_cost) }}" required class="mt-1 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        ...
                        <select name="associated_website" class="mt-1 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-blue-500">
                            ...
                            @foreach($websites as $id => $websiteName)
                                <option value="{{ $id }}" {{ old('associated_website', $email->associated_website) == $id ? 'selected' : '' }}>{{ $websiteName }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Start Date</label>
                        <input type="date" name="start_date" value="{{ old('start_date', optional($email->start_date)->format('Y-m-d')) }}" required class="mt-1 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Renewal Date</label>
                        <input type="date" name="renewal_date" value="{{ old('renewal_date', optional($email->renewal_date)->format('Y-m-d')) }}" required class="mt-1 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="4" class="mt-1 w-full rounded-xl border-gray-300 focus:ring-2 focus:ring-blue-500">{{ old('notes', $email->notes) }}</textarea>
                </div>
