...
<div class="p-6 space-y-6 max-w-3xl">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('websites.show', $website) }}" class="text-gray-400 hover:text-gray-600">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Website</h1>
            <p class="text-sm text-gray-500 mt-0.5">{{ $website->name }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('websites.update', $website) }}" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
            ...
            <div class="px-5 py-4 grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input name="name" value="{{ old('name', $website->name) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                <div>
                    ...
                    <select name="status" class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        @foreach(['active','inactive','maintenance'] as $s)
                            <option value="{{ $s }}" {{ old('status', $website->status)===$s?'selected':'' }}>{{ ucfirst($s) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Client Name</label>
                    <input name="client_name" value="{{ old('client_name', $website->client_name) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Client Email</label>
                    <input type="email" name="client_email" value="{{ old('client_email', $website->client_email) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Host Server</label>
                    <input name="host_server" value="{{ old('host_server', $website->host_server) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Deployment Date</label>
                    <input type="date" name="deployment_date" value="{{ old('deployment_date', optional($website->deployment_date)->format('Y-m-d')) }}" required class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                <div class="md:col-span-2">
                    <label class="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Description</label>
                    <textarea name="description" rows="3" class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $website->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="px-5 py-4 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input name="domain" value="{{ old('domain', $website->domain) }}" required placeholder="example.com" class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                    </div>
                    <div>
                ...
                <div class="flex flex-col gap-2.5">
                    ...
                    <label class="inline-flex items-center gap-2.5 text-sm font-medium text-gray-700 cursor-pointer select-none">
                        <input id="domain_purchased" type="checkbox" name="domain_purchased" value="1" {{ old('domain_purchased', $website->domain_purchased) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        I purchased this domain
                    </label>
                    <input type="hidden" name="amount_includes_domain" value="0">
                    <label class="inline-flex items-center gap-2.5 text-sm font-medium text-gray-700 cursor-pointer select-none">
                        <input type="checkbox" name="amount_includes_domain" value="1" {{ old('amount_includes_domain', $website->amount_includes_domain) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                ...
                <div id="domain-cost-input-wrap" class="bg-gray-50 rounded-xl border border-gray-100 px-5 py-4 space-y-3">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <input id="domain_base_cost" type="number" step="0.01" min="0" name="domain_base_cost" value="{{ old('domain_base_cost', $website->domain_base_cost) }}" placeholder="17.00" class="w-full rounded-lg border-gray-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                        </div>
                        <div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('websites.show', $website) }}" class="px-4 py-2 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">Cancel</a>
            <button type="submit" class="px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-sm font-medium text-white transition-colors">Update Website</button>
        </div>
    </form>
