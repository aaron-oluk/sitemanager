            ...
            <form method="POST" action="{{ route('websites.update', $website) }}" class="bg-white rounded shadow border border-gray-100 p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input name="name" value="{{ old('name', $website->name) }}" required class="mt-1 w-full rounded-sm border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Domain</label>
                        <input name="domain" value="{{ old('domain', $website->domain) }}" required class="mt-1 w-full rounded-sm border-gray-300 focus:ring-2 focus:ring-blue-500" placeholder="example.com" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input name="host_server" value="{{ old('host_server', $website->host_server) }}" required class="mt-1 w-full rounded-sm border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Deployment Date</label>
                        <input type="date" name="deployment_date" value="{{ old('deployment_date', optional($website->deployment_date)->format('Y-m-d')) }}" required class="mt-1 w-full rounded-sm border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Amount Paid</label>
                        <input type="number" step="0.01" name="amount_paid" value="{{ old('amount_paid', $website->amount_paid) }}" required class="mt-1 w-full rounded-sm border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input name="client_name" value="{{ old('client_name', $website->client_name) }}" required class="mt-1 w-full rounded-sm border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Client Email</label>
                        <input type="email" name="client_email" value="{{ old('client_email', $website->client_email) }}" required class="mt-1 w-full rounded-sm border-gray-300 focus:ring-2 focus:ring-blue-500" />
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Description</label>
                    <textarea name="description" rows="4" class="mt-1 w-full rounded-sm border-gray-300 focus:ring-2 focus:ring-blue-500">{{ old('description', $website->description) }}</textarea>
                </div>
